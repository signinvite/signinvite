...
type WhitelistedMailDeleteDialogProps = {
  email?: string;
  open: boolean;
...
export const WhitelistedMailDeleteDialog = ({
  email,
  open,
  onOpenChange,
  onDelete,
}: WhitelistedMailDeleteDialogProps) => {
  ...
  const { mutateAsync: deleteWhitelistedMail, isPending } =
    trpcReact.admin.deleteWhitelistedMail.useMutation({
      onSuccess: async () => {
        void refreshLimits();

        toast({
          title: _(msg`Email Deleted`),
          description: _(msg`The email has been successfully deleted.`),
          duration: 5000,
        });
    ...
    <Dialog open={open} onOpenChange={(value) => !isPending && onOpenChange(value)}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            <Trans>You are about to delete this email</Trans>
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button
            type="button"
            loading={isPending}
            onClick={() => email && void deleteWhitelistedMail({ email })}
            variant="destructive"
          >
            <Trans>Delete</Trans>
          </Button>
